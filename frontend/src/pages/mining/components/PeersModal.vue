    ...
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] flex flex-col">
      <div class="flex items-center justify-between p-4 border-b border-slate-200">
        <div class="flex items-center gap-2">
          <div class="w-9 h-9 bg-slate-800/10 rounded-full flex items-center justify-center">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        >
          <div class="w-3 h-3 rounded-full bg-slate-800 animate-pulse flex-shrink-0"></div>
          <div class="flex-1 min-w-0">
            <div class="font-medium text-slate-800 truncate">
              {{ peer || 'Unknown User' }}
              <span v-if="peer && peer === currentMiner" class="text-xs text-slate-800 ml-2 font-normal">(나)</span>
            </div>
            <div class="text-xs text-slate-500">온라인</div>
          </div>
          <div v-if="peer && peer === currentMiner" class="w-6 h-6 bg-slate-800/10 rounded-full flex items-center justify-center flex-shrink-0">
