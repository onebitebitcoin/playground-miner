        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <div class="font-medium text-slate-800 truncate">
              {{ peer || 'Unknown User' }}
              <span v-if="peer && peer === currentMiner" class="text-xs text-emerald-600 ml-2 font-normal">(나)</span>
            </div>
            <div class="text-xs text-slate-500">온라인</div>
          </div>
          <div v-if="peer && peer === currentMiner" class="w-6 h-6 bg-emerald-100 rounded-full flex items-center justify-center flex-shrink-0">
